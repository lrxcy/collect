...
type WorkRequest struct {
	Execute      func(config interface{}) (interface{}, error)
...
func (w Worker) Start() {
	go func() {
		for {
			...
			select {
			case work := <-w.Work:
				result, err := work.Execute(work.ExecuteInput)
				if err != nil {
					// log.Printf("Recive error %v", err)
					work.Result <- err
				} else {
					// log.Printf("Processing %v", result)
					work.Result <- result
				}
			case <-w.QuitChan:
...
func (d *DispatcherPool) StartDispatcher() error {
	if d.nworkers <= 0 {
		return fmt.Errorf("Invalid number of workers : %v", d.nworkers)
	}
