...
func StartMySQLDB() error {
	...
	for _, t := range tables {
		if err := mysqlDb.migration(t); err != nil {
			return err
		}
	}
...
type MySQLDBAccessObject interface {
	migration(interface{}) error
	Close() error
	ExecSql(string) error
	CreateRecord(string, ...string) error
	QueryRecord(string) (*[]map[string]interface{}, error)

	// CreateGameRecords 以string格式放入原始資料
...
func initMySqlDB(c *MySQLConfig) (MySQLDBAccessObject, error) {
	...
	if os.Getenv("TestDB") == "test" {
		dbtype = "sqlite3"
		c.DBUri = "/tmp/" + fmt.Sprintf("%v", time.Now().Unix()) + "_gorm.db"
	}
...
*/
func (db *mysqlDBObj) migration(v interface{}) error {
	return db.DB.AutoMigrate(v).Error
}
