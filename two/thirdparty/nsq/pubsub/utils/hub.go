...
// (uinqId, topic, addrNsqdHTTP string, addrNsqdTCP, addrNsqlookupd *string) (*NsqTopicReader, error) {
func (h *hub) run(uinqId, addrNsqdHTTP string, addrNsqdTCP, addrNsqlookupd *string) {
	...
	for {
		...
		case sub := <-h.addSubscriber:
			...
			if !is_reader_exists {
				var err error
				reader, err = NewNsqTopicReader(uinqId, sub.Topic, addrNsqdHTTP, addrNsqdTCP, addrNsqlookupd)
				if err != nil {
